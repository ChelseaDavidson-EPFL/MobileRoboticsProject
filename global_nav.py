...
def display_grid(map_grid, start=None, goal=None):
    ...
    map_display = np.zeros_like(map_grid, dtype=int)

    # Colors - obstacles in RED (index 2)
    map_display[map_grid == -1] = 2  # Red for obstacles
    map_display[map_grid == 0] = 0   # White for free space

    if start is not None:
        map_display[start] = 3  # Blue for start
    if goal is not None:
        map_display[goal] = 4  # Green for goal
...
def expand_grid_by_robot(grid, robot_size_cells, safety_margin=0):
    ...
    display_grid(expanded_grid, None, None)
